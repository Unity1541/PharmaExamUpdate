...
### 8.4 資料載入與快取策略 (Data Loading & Caching Strategy)

為兼顧**使用者體驗 (UX)** 與 **Firestore 讀取成本**，系統針對不同角色採取了差異化的資料載入策略：

#### 1. 學生端：極致懶載入 (Lazy Loading)
*   **登入時（無感載入）：** 學生登入時，**不載入任何題庫資料**，僅讀取該學生的基本資料 (`users` 文件) 與考試紀錄 (`examHistory`)。這保證了 99% 流量的登入門戶能夠「秒屏」顯示。
*   **測驗時（按需取用）：** 當學生點擊「開始測驗」並選擇科目/類別後，系統才發送查詢請求，精準拉取該類別的數十道題目。
*   **記憶體快取（In-Memory Cache）：** 凡是從 Firestore 下載過的題目，全部會被推進 `state.allQuestions` 陣列中快取。
    *   **二次讀取零延遲：** 若學生退出後再進入相同類別測驗，或在「歷史測驗回顧」中遇到曾下載過的題目，均直接從 `state.allQuestions` 讀取，完全不耗費 Firestore 讀取額度。
    *   **動態補齊：** 查看「歷史測驗回顧」時，若考卷中包含未曾載入（快取中無資料）的題目，系統會自動發送 `getDoc` 請求，精準拉取缺漏的題目定型並推入快取。

#### 2. 管理員端：全量載入 (Front-Loading)
*   **登入時（全表掃描）：** 管理員因需綜觀全局與編輯題庫，登入時會執行 `getDocs` 全量拉取 `questions` 以及 `users` 集合。
*   這是目前設計上為滿足「全局搜尋、編輯」所做的成本妥協。

#### ⚠️ 系統放大的效能隱患與未來優化方向
1.  **管理員效能瓶頸：** 若未來題庫擴增至萬題以上，管理員登入時的「全量載入」將導致嚴重的網路延遲與高昂的讀取費用。
    *   **優化方案：** 管理員後台的「題庫管理」與「用戶管理」列表需改寫為**分頁機制 (Pagination)**，搭配 Firestore 的 `limit()` 與 `startAfter()` 游標查詢。
2.  **缺乏持久化快取 (Local Persistence)：** 目前快取存在於 JavaScript 記憶體中，重整網頁 (F5) 即清空重啟。
    *   **優化方案：** 未來可啟用 Firebase 官方的 **Offline Persistence (離線快取)**，將下載過的資料存入 IndexedDB。即使重新整理網頁，也能做到「近乎零網路讀取」的極速重載。
