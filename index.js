...
*/
import { auth, db, signInWithEmailAndPassword, signOut, onAuthStateChanged, collection, getDocs, addDoc, doc, getDoc, setDoc, updateDoc, deleteDoc, query, where, writeBatch, onSnapshot } from './firebase.js';
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const setState = (newState) => {
      ...
      if (newState.currentView && newState.currentView !== 'student-analytics' && adminStudentHistoryListener) {
    ...
    async function handleDeleteSubject(id, name) {
        if (!confirm(`確定要刪除科目 "${name}" 嗎？\n這將同時刪除該科目下的所有類別。`)) return;
        setLoading(true);
        try {
            await deleteDoc(doc(db, "subjects", id));
            // Find and delete categories for this subject
            const q = query(collection(db, "categories"), where("subject", "==", name));
    ...
    async function handleDeleteCategory(id, subjectName) {
        ...
        try {
            ...
            const updatedSubjectCats = state.categories[subjectName].filter(c => c.id !== id);
            setState({ categories: { ...state.categories, [subjectName]: updatedSubjectCats } });
        } catch (e) {
            console.error(e);
            alert("刪除失敗");
        } finally {
    ...
    async function handleBulkUpload() {
         if (!state.selectedFile || !state.selectedBulkUploadSubject || !state.selectedBulkUploadCategory) {
             alert("請選擇科目、類別和檔案");
             return;
         }
         ...
         reader.onload = async (e) => {
             try {
                 ...
                 json.forEach(item => {
                     // Basic validation
                     if (!item.text || !item.options || item.options.length !== 4 || !item.answer) {
                         // skip invalid
                         return;
                     }
                 ...
                 console.error(err);
                 setState({ uploadStatus: 'error', uploadMessage: "上傳失敗：" + err.message });
             } finally {
                 setLoading(false);
             }
         };
    ...
    async function handleUpdateUser(e) {
         ...
         try {
             ...
             const updatedStudents = state.allStudents.map(s => s.id === u.id ? {...s, name: newName} : s);
    ...
    async function handleDeleteUser(id) {
         if (!confirm("確定要刪除此使用者嗎？這將會一並刪除該使用者的所有測驗紀錄、收藏與作業資料。")) return;
         setLoading(true);
         try {
             // 1. Delete Exam History
             const historyQ = query(collection(db, "examHistory"), where("userId", "==", id));
             ...
             // 2. Delete Bookmarks
             const bookmarkQ = query(collection(db, "bookmarkedQuestions"), where("userId", "==", id));
             ...
             const submissionQ = query(collection(db, "assignmentSubmissions"), where("userId", "==", id));
    ...
    async function handleDeleteExamHistory(id) {
        ...
        try {
            ...
            if (state.selectedStudentAnalyticsData) {
                 const newHistory = state.selectedStudentAnalyticsData.examHistory.filter(h => h.id !== id);
                 ...
                 const newHistory = state.currentUser.examHistory.filter(h => h.id !== id);
                 const newRadar = calculateRadarData(newHistory);
                 setState({ 
                    currentUser: { 
                        ...state.currentUser, 
    ...
    async function handleDeleteBookmark(questionId, userId) {
         ...
         if (userId === state.currentUser.id) {
             const b = state.currentUser.bookmarkedQuestions.find(bq => bq.id === questionId);
             if (b) bookmarkId = b.bookmarkId;
         } else if (state.selectedStudentAnalyticsData) {
             const b = state.selectedStudentAnalyticsData.bookmarkedQuestions.find(bq => bq.id === questionId);
             if (b) bookmarkId = b.bookmarkId;
         }
         ...
         if (bookmarkId) {
             ...
             try {
                 ...
                 if (userId === state.currentUser.id) {
                      const newB = state.currentUser.bookmarkedQuestions.filter(b => b.bookmarkId !== bookmarkId);
                      setState({ currentUser: { ...state.currentUser, bookmarkedQuestions: newB } });
                 } else {
                      const newB = state.selectedStudentAnalyticsData.bookmarkedQuestions.filter(b => b.bookmarkId !== bookmarkId);
                      setState({ selectedStudentAnalyticsData: { ...state.selectedStudentAnalyticsData, bookmarkedQuestions: newB } });
                 }
    ...
    async function handleAddAssignment(e) {
        ...
        questionBlocks.forEach(block => {
            ...
            const image = block.querySelector('input[name="imageUrl"]').value.trim() || null;
            const score = parseInt(block.querySelector('input[name="maxScore"]').value, 10) || 0;
            if(text) {
    ...
    async function handleAssignmentSubmit(status = 'submitted') {
         ...
         if (status === 'submitted' && !confirm("確定要提交作業嗎？提交後無法修改。")) return;
         ...
         try {
             ...
             if (status === 'submitted') {
                 alert("作業已提交！");
                 setState({ currentView: 'handwritten-assignments', submissionDraft: null, currentAssignment: null });
             } else {
                 alert("草稿已儲存！");
    ...
    async function handleAdminGradeSubmission(e) {
        ...
        try {
             ...
             const updatedSubmissions = state.assignmentSubmissions.map(sub => 
                sub.id === submissionId ? { ...sub, score, feedback, feedbackImageUrl, status: 'graded' } : sub
             );
    ...
    function startExam(subject, categoryName) {
         setLoading(true);
         const category = state.categories[subject].find(c => c.name === categoryName);
         if (!category) { alert("類別錯誤"); setLoading(false); return; }
         
         // Filter questions
         const questions = state.allQuestions.filter(q => q.subject === subject && q.category === categoryName);
         if (questions.length === 0) { alert("此類別暫無題目"); setLoading(false); return; }
    ...
    function handleAnswerSelection(questionId, optionIndex) {
         if (!state.examState) return;
         const newAnswers = { ...state.examState.answers, [questionId]: optionIndex };
    ...
    async function handleBookmarkToggle(questionId) {
         ...
         const existing = state.currentUser.bookmarkedQuestions.find(q => q.id === questionId);
         setLoading(true);
         try {
             if (existing) {
                 await deleteDoc(doc(db, "bookmarkedQuestions", existing.bookmarkId));
                 const newBookmarks = state.currentUser.bookmarkedQuestions.filter(q => q.id !== questionId);
                 setState({ currentUser: { ...state.currentUser, bookmarkedQuestions: newBookmarks } });
             } else {
                 ...
                 const newBookmarks = [...state.currentUser.bookmarkedQuestions, { ...qDetails, bookmarkId: docRef.id }];
                 setState({ currentUser: { ...state.currentUser, bookmarkedQuestions: newBookmarks } });
             }
         } catch(e) { console.error(e); } finally { setLoading(false); }
    }
    ...
    function createSidebarHTML() {
        ...
        const navLinks = role === 'student' ? `
            ...
            <li><a href="#" id="nav-handwritten" class="${state.currentView.startsWith('handwritten') || state.currentView === 'do-assignment' ? 'active' : ''}">${icons.pencil} 手寫作業區</a></li>
        ` : `
            ...
            <li><a href="#" id="nav-admin-assignments" class="${state.currentView.startsWith('admin-') && state.currentView.includes('assignment') ? 'active' : ''}">${icons.pencil} 手寫作業管理</a></li>
        `;
    ...
    function createRadarChartView(data) {
        ...
        if (!data || data.length === 0) return '<div class="radar-chart-container"><p>尚無數據</p></div>';
        ...
        for (let i = 1; i <= 4; i++) {
            ...
            gridHTML += `<polygon points="${points.join(' ')}" class="radar-grid-line" />`;
        }
        ...
        return `
            <div class="radar-chart-container">
                <svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
                    ${gridHTML}
                    <polygon points="${dataPoints.join(' ')}" class="radar-shape" />
                    ${labelsHTML}
                </svg>
            </div>
        `;
    ...
    function createStudentDashboardViewHTML(user) {
        ...
        const avgScore = examCount > 0 
            ? Math.round(user.examHistory.reduce((acc, curr) => acc + curr.score, 0) / examCount) 
            : 0;

        const historyItems = user.examHistory ? user.examHistory.slice(0, 5).map(h => `
            <li class="detail-list-item history-item">
                <div class="item-main-info">
                    <span class="item-subject">${h.subject} - ${h.category}</span>
                    <span class="item-score">得分: ${h.score}分 | 日期: ${new Date(h.date).toLocaleDateString()}</span>
                </div>
                <div class="history-item-meta">
                    <button class="review-button" data-exam-id="${h.id}">查看詳情</button>
                </div>
            </li>
        `).join('') : '';
        
        const bookmarkItems = user.bookmarkedQuestions ? user.bookmarkedQuestions.map(b => `
            <li class="detail-list-item question-item clickable-bookmark" onclick="window.openBookmarkDetail('${b.id}')">
                <div style="display:flex; justify-content:space-between; width:100%;">
                    <span class="item-subject-badge">${b.subject}</span>
                    <button class="delete-bookmark-btn" data-question-id="${b.id}" style="border:none; background:none; cursor:pointer; color:#999;">&times;</button>
                </div>
                <p class="item-text">${b.text.substring(0, 60)}...</p>
            </li>
        `).join('') : '';
        ...
        return `
            ...
            <div class="dashboard-details-grid">
                <div class="detail-card">
                    ...
                    ${createRadarChartView(user.radarChartData)}
                </div>
                ...
                <div class="detail-card">
                    ...
                    <ul class="detail-list">
                        ${historyItems || '<p class="empty-list-message">尚無測驗紀錄</p>'}
                    </ul>
                </div>
                ...
                <div class="detail-card">
                    ...
                    <ul class="detail-list scrollable-list">
                         ${bookmarkItems || '<p class="empty-list-message">尚無收藏</p>'}
                    </ul>
                </div>
            </div>
        `;
    ...
    function createExamSelectionViewHTML() {
        const subjectCards = state.subjects.map(subject => {
            // Count questions available
            const count = state.allQuestions.filter(q => q.subject === subject.name).length;
            return `
    ...
    function createExamTakingViewHTML() {
        if (!state.examState) return '';
        const { subject, category, questions, currentQuestionIndex, timeLeft, answers } = state.examState;
        const currentQ = questions[currentQuestionIndex];
        const progressPercent = ((Object.keys(answers).length) / questions.length) * 100;
        
        // Options HTML
        const opts = currentQ.options.map((optText, idx) => {
             const isSelected = answers[currentQ.id] === idx;
             const img = currentQ.optionImages && currentQ.optionImages[idx] ? `<img src="${currentQ.optionImages[idx]}" class="option-image">` : '';
             return `
                <li class="exam-option ${isSelected ? 'selected' : ''}" onclick="handleAnswerSelection('${currentQ.id}', ${idx})">
                    <div class="option-letter">${['A','B','C','D'][idx]}</div>
                    <div>${optText}</div>
                    ${img}
                </li>
             `;
        ...
        // Nav Grid HTML
        const navGrid = questions.map((q, idx) => {
             const isCurrent = idx === currentQuestionIndex;
             const isAnswered = answers[q.id] !== undefined;
             const isBookmarked = state.currentUser.bookmarkedQuestions.some(bq => bq.id === q.id);
             ...
             return `<div class="${className}" onclick="handleJumpToQuestion(${idx})">${idx + 1}</div>`;
        }).join('');
                ...
                <div class="exam-main-panel">
                    <div class="page-header">
                         ...
                         <div id="exam-timer-display" style="font-size: 20px; font-weight:700; color: #e57373; display:flex; align-items:center; gap:6px;">
                            ${icons.clock} <span id="exam-timer-span">${formatTime(timeLeft)}</span>
                         </div>
                    </div>
                    ...
                    <div class="exam-question-card">
                         <div class="question-header-row">
                             <div class="question-label">Q${currentQuestionIndex + 1}</div>
                             <div style="display:flex; align-items:center;">
                                <button class="bookmark-btn ${state.currentUser.bookmarkedQuestions.some(b => b.id === currentQ.id) ? 'active' : ''}" onclick="handleBookmarkToggle('${currentQ.id}')">
                                    ${state.currentUser.bookmarkedQuestions.some(b => b.id === currentQ.id) ? icons.bookmarkSolid : icons.bookmark}
                                </button>
                             </div>
                         </div>
                         <div class="exam-question-text">${currentQ.text}</div>
                         ${currentQ.imgurl ? `<img src="${currentQ.imgurl}" class="question-image">` : ''}
                         ...
                         <div class="exam-navigation">
                            <button class="exam-nav-button" onclick="handleQuestionNavigation(-1)" ${currentQuestionIndex === 0 ? 'disabled' : ''}>${icons.arrowLeft} 上一題</button>
                            ${isLast 
    ...
    function createReviewExamViewHTML(exam) {
        ...
        if (!exam.questions || exam.questions.length === 0) {
            ...
            content = exam.questions.map((qId, idx) => {
                 const q = state.allQuestions.find(i => i.id === qId);
                 if (!q) return `<div class="review-question-card"><p>題目 ID: ${qId} (已刪除)</p></div>`;
                 
                 const userAnsIndex = exam.answers[qId];
                 const correctText = q.answer;
                 const userAnsText = userAnsIndex !== undefined ? q.options[userAnsIndex] : '未作答';
                 const isCorrect = userAnsText === correctText;
                 
                 return `
                    <div class="review-question-card">
                        <div class="review-question-text"><span style="font-weight:bold; margin-right:8px;">${idx+1}.</span> ${q.text}</div>
                        ${q.imgurl ? `<img src="${q.imgurl}" class="question-image" style="max-height:200px;">` : ''}
                        
                        <div class="review-summary">
                            <div>您的答案: <span class="summary-answer ${isCorrect ? 'correct-text' : 'incorrect-text'}">${userAnsText}</span></div>
                        ...
                        <div class="explanation-box">
                            ...
                            ${q.explanationImage ? `<div style="margin-top:10px;"><img src="${q.explanationImage}" style="max-width:100%; border-radius:4px;"></div>` : ''}
                        </div>
                    </div>
                 `;
            }).join('');
    ...
    function createAdminViewHTML() {
        ...
        if (state.currentView === 'user-management') {
            const list = state.allStudents.map(u => `
            ...
            return `
                <div class="admin-view-container">
                    ...
                    <div class="dashboard-container">
                        <h3>使用者列表</h3>
                        <ul class="detail-list scrollable-list">${list || '<p class="empty-list-message">無使用者</p>'}</ul>
                    </div>
                </div>`;
        }
        ...
        if (state.currentView === 'subject-management') {
             const list = state.subjects.map(s => {
        ...
        if (state.currentView === 'question-editing') {
             const filtered = state.allQuestions.filter(q => {
                 if (state.selectedAdminSubject && q.subject !== state.selectedAdminSubject) return false;
                 if (state.selectedAdminCategory && q.category !== state.selectedAdminCategory) return false;
                 return true;
             });
             ...
             const list = filtered.map(q => `
                <li class="admin-list-item detail-list-item">
                    <div class="item-main-info">
                        ...
                        <p class="item-text" style="margin-top:4px;">${q.text.substring(0, 80)}...</p>
                    </div>
                    <div class="actions">
             ...
             const catOptions = state.selectedAdminSubject && state.categories[state.selectedAdminSubject] 
                ? state.categories[state.selectedAdminSubject].map(c => `<option value="${c.name}">${c.name}</option>`).join('')
                    ...
                    <div class="dashboard-container">
                        <div class="admin-filters">
                             <select class="filter-select" onchange="window.handleAdminSubjectFilterChange(this.value)">
                                ...
                                ${state.subjects.map(s => `<option value="${s.name}" ${state.selectedAdminSubject === s.name ? 'selected' : ''}>${s.name}</option>`).join('')}
                             </select>
                             <select class="filter-select" onchange="window.handleAdminCategoryFilterChange(this.value)" ${!state.selectedAdminSubject ? 'disabled' : ''}>
                                <option value="">所有類別</option>
                                ${state.selectedAdminSubject ? state.categories[state.selectedAdminSubject].map(c => `<option value="${c.name}" ${state.selectedAdminCategory === c.name ? 'selected' : ''}>${c.name}</option>`).join('') : ''}
                             </select>
                        </div>
                        <ul class="detail-list scrollable-list" style="max-height:600px;">
                            ${list || '<p class="empty-list-message">無題目符合條件</p>'}
                        </ul>
                    </div>
                </div>
             `;
        ...
        if (state.currentView === 'bulk-upload') {
             const catOptions = state.selectedBulkUploadSubject && state.categories[state.selectedBulkUploadSubject]
                ? state.categories[state.selectedBulkUploadSubject].map(c => `<option value="${c.name}">${c.name}</option>`).join('')
                : '<option value="">請先選擇科目</option>';
                    ...
                    <div class="dashboard-container upload-card-body">
                         <div class="admin-filters">
                             <select class="filter-select" onchange="window.handleBulkUploadSubjectChange(this.value)">
                                ...
                                ${state.subjects.map(s => `<option value="${s.name}" ${state.selectedBulkUploadSubject === s.name ? 'selected' : ''}>${s.name}</option>`).join('')}
                             </select>
                             <select class="filter-select" onchange="window.handleBulkUploadCategoryChange(this.value)" ${!state.selectedBulkUploadSubject ? 'disabled' : ''}>
                                <option value="">選擇類別</option>
                                ${state.selectedBulkUploadSubject ? state.categories[state.selectedBulkUploadSubject].map(c => `<option value="${c.name}" ${state.selectedBulkUploadCategory === c.name ? 'selected' : ''}>${c.name}</option>`).join('') : ''}
                             </select>
                        </div>
...
[
  {
    ...
    "explanationImage": "/images/explanation.jpg (選填，詳解圖片)"
        ...
        if (state.currentView === 'student-analytics') {
             const userSelect = `
                <div class="dashboard-container" style="margin-bottom:24px;">
                    ...
                    <select class="filter-select" onchange="window.handleStudentAnalyticsSelect(this.value)" style="width:100%;">
                        ...
                        ${state.allStudents.filter(u => u.role === 'student').map(u => `<option value="${u.id}" ${state.selectedStudentIdForAnalytics === u.id ? 'selected' : ''}>${u.name} (${u.email})</option>`).join('')}
                    </select>
                </div>
             `;
             ...
             if (state.selectedStudentAnalyticsData) {
                 ...
                 const historyItems = u.examHistory.map(h => `
                    <li class="detail-list-item history-item">
                        <div class="item-main-info">
                            <span class="item-subject">${h.subject} - ${h.category}</span>
                            <span class="item-score">得分: ${h.score} | ${new Date(h.date).toLocaleDateString()}</span>
                        </div>
                        <div class="history-item-meta">
                            <button class="review-button" data-exam-id="${h.id}">查看</button>
                            <button class="action-btn delete delete-history-btn" data-history-id="${h.id}">&times;</button>
                        </div>
                    </li>
                 `).join('');
                 ...
                 analyticsContent = `
                    <div class="dashboard-details-grid full-layout">
                         ...
                         <div class="detail-card">
                             ...
                             <ul class="detail-list scrollable-list">${historyItems || '無紀錄'}</ul>
                         </div>
                    </div>
                 `;
             }
    ...
    function createHandwrittenAssignmentListViewHTML() {
        const listItems = state.assignments.map(a => {
            const firstQuestionText = a.questions ? a.questions[0].text : (a.content || '');
            return `
            <div class="subject-card">
                <h3>${a.title}</h3>
                <p class="description">${firstQuestionText.substring(0, 50)}...</p>
                <div class="card-footer">
                     <span class="question-count">${a.subject} ${a.category ? ` - ${a.category}` : ''} | 配分: ${a.maxScore}</span>
                     <button class="select-button" onclick="window.openAssignment('${a.id}')">進入作答 ${icons.arrowRight}</button>
                </div>
            </div>`;
        }).join('');
    ...
    function createDoAssignmentViewHTML() {
        ...
        const isSubmitted = sub && sub.status === 'submitted' || sub && sub.status === 'graded';
        const isGraded = sub && sub.status === 'graded';
        
        // Handle Legacy Data: If no 'questions' array, wrap 'content' into an array
        const questions = a.questions || [{ text: a.content, image: a.imageUrl, score: a.maxScore }];
        ...
        const assignmentItems = questions.map((q, idx) => {
                        ...
                        <div class="hw-card hw-question-card">
                            ...
                            <div class="hw-card-body">
                                <div class="hw-question-content">
                                    ${q.text}
                                    ${q.image ? `<div style="margin-top:16px;"><img src="${q.image}" style="max-width:100%; border-radius:8px; border:1px solid #eee;"></div>` : ''}
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="hw-card hw-answer-card h-full">
                            ...
                            <div class="hw-card-body flex-col">
                                ...
                                <div 
                                    ...
                                    id="${uniqueEditorId}"
                                    data-placeholder="在此輸入第 ${idx + 1} 題答案..."
                                    style="${isSubmitted ? 'background-color: #f9fafb;' : ''}"
                                >${savedContent}</div>
        ...
        return `
            <div class="assignment-view-container">
                <header class="page-header" style="padding: 24px 48px;">
                    ...
                    <button class="back-button" onclick="window.goBackToAssignments()">${icons.arrowLeft} 返回列表</button>
                </header>
                ...
                <div style="flex-grow: 1; overflow-y: auto; padding-bottom: 40px; padding-left: 48px; padding-right: 48px;">
                    ...
                    ${isGraded ? `
                        <div class="grading-card">
                            <h3><span style="color:var(--primary-color-dark)">✓ 已評分</span> - 得分：${sub.score} / ${a.maxScore}</h3>
                            <p style="margin-top:8px;"><strong>老師評語：</strong></p>
                            <p style="white-space: pre-wrap; color: var(--text-color); margin-bottom:16px;">${sub.feedback}</p>
                ...
                </div>` : '<div class="submit-section" style="position: fixed; bottom: 0; right: 0; width: calc(100% - 260px); background: var(--bg-color); padding: 16px;"><p style="color:var(--text-light); text-align:right;">作業已提交。</p></div>'}
            </div>
        `;
    }
    ...
    function createAdminAssignmentViewsHTML() {
            ...
            const listItems = state.assignments.map(a => `
                <li class="admin-list-item detail-list-item">
                    <div class="item-main-info">
                        ...
                        <p class="item-text" style="font-size:12px;">${a.subject} ${a.category ? ` - ${a.category}` : ''} | 配分: ${a.maxScore}</p>
                    </div>
                     <div class="actions">
                        <button class="action-btn" title="查看提交" onclick="window.openAdminGradeList('${a.id}')">${icons.users}</button>
                        <button class="action-btn delete" title="刪除作業" onclick="window.handleDeleteAssignment('${a.id}')">${icons.delete}</button>
                    </div>
                </li>
            `).join('');
            ...
            return `
                <div class="admin-view-container">
                    ...
                    <div class="management-grid">
                        <div class="dashboard-container">
                             <h3>作業列表</h3>
                             <ul class="detail-list scrollable-list">${listItems || '<p class="empty-list-message">尚無作業</p>'}</ul>
                        </div>
                        <div class="dashboard-container">
                            <h3>新增作業</h3>
        ...
        if (state.currentView === 'admin-grade-assignment') {
            const subItems = state.assignmentSubmissions.map(s => {
                let statusBadge = '';
                if (s.status === 'graded') statusBadge = `<span style="color:green; font-weight:bold;">${s.score}分</span>`;
                else statusBadge = `<span style="color:orange;">未評分</span>`;
                
                return `
                <li class="admin-list-item detail-list-item">
                    <div class="item-main-info">
                        <span class="item-subject">${s.userName || 'Unknown Student'}</span>
                        <div class="item-meta">Submitted: ${new Date(s.updatedAt || s.createdAt).toLocaleDateString()}</div>
                    </div>
                     <div style="display:flex; align-items:center; gap:12px;">
                        ${statusBadge}
                        <button class="action-btn" title="評分" onclick="window.openGradingDetail('${s.id}')">${icons.edit}</button>
                    </div>
                </li>
            `}).join('');
            ...
            return `
                 <div class="admin-view-container">
                    <header class="page-header">
                        <div><h2>作業評分列表</h2><p>${state.currentAssignment ? state.currentAssignment.title : ''}</p></div>
                        <button class="back-button" onclick="setState({currentView: 'admin-assignments', currentAssignment: null})">${icons.arrowLeft} 返回作業管理</button>
                    </header>
                    <div class="dashboard-container">
                        <ul class="detail-list scrollable-list">${subItems.length > 0 ? subItems : '<p class="empty-list-message">尚無提交記錄</p>'}</ul>
                    </div>
                 </div>
            `;
        }
        ...
        if (state.currentView === 'admin-grading-detail') {
             ...
             const questions = assign.questions || [{ text: assign.content, image: assign.imageUrl, score: assign.maxScore }];
                ...
                <div class="assignment-item">
                    ...
                    <div class="hw-card">
                         <div class="hw-card-body">
                            ...
                            <div class="editor-area" style="border:none; background:white;">${studentAnswers[idx] || '<span style="color:red">未作答</span>'}</div>
                         </div>
                    </div>
                </div>
             `).join('');
             ...
             return `
                 <div class="admin-view-container">
                    <header class="page-header">
                        ...
                        <button class="back-button" onclick="setState({currentView: 'admin-grade-assignment', gradingSubmission: null})">${icons.arrowLeft} 返回列表</button>
                    </header>
                        ...
                        <div class="dashboard-container">
                            ...
                            <form id="grading-form" class="admin-form">
                                <div class="form-group">
                                    <label>得分 (滿分: ${assign.maxScore})</label>
                                    <input type="number" name="score" max="${assign.maxScore}" value="${sub.score || ''}" required>
                                </div>
                                <div class="form-group">
                                    <label>評語</label>
                                    <textarea name="feedback" rows="6">${sub.feedback || ''}</textarea>
                                </div>
                                <div class="form-group">
                                    <label>批改圖片路徑 (選填)</label>
                                    <input type="text" name="feedbackImageUrl" value="${sub.feedbackImageUrl || ''}" placeholder="/images/corrections/sub_123.jpg">
    ...
    function render() {
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="review-question-card" style="border:none; padding:0;">
                                ...
                                ${q.imgurl ? `<img src="${q.imgurl}" class="question-image">` : ''}
                                
                                <ul class="review-options-list">
                                    ${q.options.map((opt, idx) => {
                                        const isCorrect = opt === q.answer;
                                        return `<li class="review-option ${isCorrect ? 'correct' : ''}">
                                            <div style="font-weight:bold; margin-right:8px;">${['A','B','C','D'][idx]}.</div>
                                            <div>${opt}</div>
                                        </li>`;
                                    }).join('')}
                                </ul>
                    ...
                    <div class="modal-content" style="height: auto; max-height: 90vh;">
                        ...
                        <div class="modal-body">
                            <form id="edit-question-form" class="admin-form">
                                ...
                                <div class="form-group"><label>選項 1</label><input type="text" name="option1" value="${q.options[0]}" required><input type="text" name="option1_img" placeholder="選項 1 圖片網址 (選填)" value="${optImgs[0] || ''}" class="mt-1"></div>
                                <div class="form-group"><label>選項 2</label><input type="text" name="option2" value="${q.options[1]}" required><input type="text" name="option2_img" placeholder="選項 2 圖片網址 (選填)" value="${optImgs[1] || ''}" class="mt-1"></div>
                                <div class="form-group"><label>選項 3</label><input type="text" name="option3" value="${q.options[2]}" required><input type="text" name="option3_img" placeholder="選項 3 圖片網址 (選填)" value="${optImgs[2] || ''}" class="mt-1"></div>
                                <div class="form-group"><label>選項 4</label><input type="text" name="option4" value="${q.options[3]}" required><input type="text" name="option4_img" placeholder="選項 4 圖片網址 (選填)" value="${optImgs[3] || ''}" class="mt-1"></div>
                                ...
                                <div class="form-group"><label>詳解圖片網址 (選填)</label><input type="text" name="explanationImage" value="${q.explanationImage || ''}"></div>
        ...
        if (state.currentUser.role === 'student') {
             ...
             document.getElementById('nav-handwritten').onclick = (e) => { e.preventDefault(); setState({ currentView: 'handwritten-assignments' }); };
        } else {
             ...
             document.getElementById('nav-admin-assignments').onclick = (e) => { e.preventDefault(); setState({ currentView: 'admin-assignments' }); };
        }
        ...
        if (state.currentUser.role === 'student') {
            ...
            // Admin Views
            if (state.currentView.startsWith('admin-') || state.currentView === 'admin-grade-assignment' || state.currentView === 'admin-grading-detail') {
        ...
        if (prevEditor && (state.currentView === 'do-assignment' || state.currentView === 'admin-grading-detail')) {
        ...
        document.querySelectorAll('.select-button').forEach(btn => {
            if(btn.dataset.subject) { // Only for exam selection, not assignment selection
        ...
        editors.forEach(editor => {
             editor.addEventListener('keydown', function(e) {
                if (e.ctrlKey || e.metaKey) {
                    if (e.key === ',') { e.preventDefault(); document.execCommand('subscript', false, null); }
                    else if (e.key === '.') { e.preventDefault(); document.execCommand('superscript', false, null); }
                }
            });
        });
    }
    ...
    window.openBookmarkDetail = (id) => {
        let q = state.currentUser?.bookmarkedQuestions?.find(item => item.id === id);
    ...
    window.goBackToAssignments = () => setState({ currentView: 'handwritten-assignments', currentAssignment: null, submissionDraft: null });
    window.openAssignment = async (assignmentId) => {
        ...
        try {
            ...
            const q = query(collection(db, 'assignmentSubmissions'), 
            ...
            setState({ currentView: 'do-assignment', currentAssignment: assignment, submissionDraft: draft });
        } catch(e) {
            alert(e.message);
        } finally {
            setLoading(false);
    ...
    window.insertSymbol = (symbol, targetId) => {
        const editor = targetId ? document.getElementById(targetId) : document.getElementById('editor');
        if (editor) {
            editor.focus();
    ...
    window.openAdminGradeList = async (assignmentId) => {
        ...
        try {
            const assignment = state.assignments.find(a => a.id === assignmentId);
            const q = query(collection(db, 'assignmentSubmissions'), where('assignmentId', '==', assignmentId));
            const snapshot = await getDocs(q);
            const submissions = snapshot.docs.map(d => ({id: d.id, ...d.data()}));
            
            setState({ currentView: 'admin-grade-assignment', currentAssignment: assignment, assignmentSubmissions: submissions });
        } catch(e) {
            alert(e.message);
        } finally {
            setLoading(false);
    ...
    window.handleStudentAnalyticsSelect = async (studentId) => {
        ...
        try {
             ...
             const qBookmarks = query(collection(db, 'bookmarkedQuestions'), where("userId", "==", studentId));
             ...
             const qHistory = query(collection(db, 'examHistory'), where("userId", "==", studentId));
             
             adminStudentHistoryListener = onSnapshot(qHistory, (snapshot) => {
                 const examHistory = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
                ...
                try {
                    ...
                    if (role === 'admin') {
                        ...
                        allStudents = usersSnapshot.docs.map(d => ({ id: d.id, ...d.data() }));
                         ...
                         let examHistory = historySnapshot.docs.map(d => ({ id: d.id, ...d.data() }));
                    ...
                    if (role === 'student') {
                         const qHistory = query(collection(db, 'examHistory'), where("userId", "==", user.uid));
                         ...
                         examHistoryListener = onSnapshot(qHistory, (snapshot) => {
                             ...
                             if (state.currentUser) {
                                 ...
                                 const updatedUser = { ...state.currentUser, examHistory: history, radarChartData: radar };
