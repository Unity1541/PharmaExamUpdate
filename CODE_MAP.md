...
| L224-297 | `handleUpdateCategory()` | 更新類別並同步題目 ⭐🔥 |
| **題目管理** | | |
...
| L453-501 | `handleUpdateCategory()` | 更新類別(名稱/時間) |
...
#### 1️⃣ 類別名稱修改同步更新題目 🔥
- **問題**: 修改類別名稱後，題目的 category 欄位未自動更新
- **解決**: handleUpdateCategory 新增批次更新功能，自動更新所有相關題目
- **技術**: 使用 Firestore writeBatch + query 批次更新

#### 2️⃣ 編輯題目時可修改類別 🔥  
- **功能**: 題目編輯 modal 新增科目（唯讀）和類別（可選）欄位
- **好處**: 管理者可輕鬆重新分類題目，提升題庫管理效率

#### 3️⃣ 原類別編輯功能
- handleUpdateCategory - 更新類別名稱和時間限制
- openEditCategoryModal / closeEditCategoryModal - Modal 控制
- 新增 editingCategory 狀態追蹤

#### 📊 累計變更
- index.js: 5,104 → 5,374 行 (+270 行)
- src/modules/admin.js: 622 → 709 行 (+87 行)
